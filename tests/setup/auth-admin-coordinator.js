...
test.describe("Admin Authentication Setup @admin-coordinator", () => {
  test("setup admin authentication", async ({ browser }) => {
    ...
    await page.getByPlaceholder("Enter email").fill(process.env.QA_ADMINCOORDINATOR_USERNAME);
    await page.getByRole("button", { name: "Next" }).click();

    // Fill password - use QA_ADMINCOORDINATOR_PASSWORD
    await page.getByPlaceholder("Enter your password").fill(process.env.QA_ADMINCOORDINATOR_PASSWORD);
    ...
    console.log("✅ Admin+Coordinator authentication state saved. (QA Environment)");
